...
const copyByLocale = {
  ja: {
    ...
    homeLead:
      "このページは完全翻訳版ではなく、各言語で集めた検索表現を入口にして、検証済みの Wisdom 質問地図へつなぐプレビューです。",
    browse: "ノードを見る",
  ...
  zh: {
    ...
    homeLead:
      "这里不是完整翻译版，而是把各语言中收集到的搜索表达作为入口，连接到已验证的 Wisdom 问题地图。",
    browse: "浏览节点",
  ...
  es: {
    ...
    homeLead:
      "Esta no es una traducción completa todavía. Usa frases reales recopiladas en cada idioma para entrar al mapa verificado de preguntas de Wisdom.",
    browse: "Ver nodos",
  ...
  fr: {
    ...
    homeLead:
      "Ce n'est pas encore une traduction complète. Cette vue utilise les expressions collectées dans chaque langue pour ouvrir la carte vérifiée de Wisdom.",
  ...
  de: {
    ...
    homeLead:
      "Dies ist noch keine vollständige Übersetzung. Die Ansicht nutzt gesammelte Suchausdrücke je Sprache als Eingang zur geprüften Wisdom-Fragenkarte.",
...
export function localizedCardBody(card, entry) {
  ...
  return `${name} gives this question a ${lens} anchor. Read it as one verified perspective connected to the canonical Wisdom card: ${entry.canonicalTitle}.`;
}
